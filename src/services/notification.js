...
async function sendSuccessNotification(recipientNames, monthName, reimbursementCount, paymentCount, bitableUrl) {
...
async function sendErrorNotification(recipientNames, monthName, errorMessage) {
    if (recipientNames.length === 0) {
        return;
    }
    const openIds = await (0, user_1.getUserOpenIdsByNames)(recipientNames);
